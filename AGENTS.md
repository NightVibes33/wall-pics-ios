...
- `fvm flutter build apk --debug` — build verification

### Secrets

Secrets are managed via **Doppler** (not `.env` files). See `docs/development/doppler.md`. Without Doppler access, targets like `make run` and `make build` will fail at the `doppler-check` step. Use direct `fvm flutter` commands when you need to bypass Makefile Doppler checks.
