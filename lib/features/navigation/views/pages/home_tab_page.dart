...
class _WallpaperSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final visibleItems = section.items.take(9).toList(growable: false);
...
class _HomeWallpaperCard extends StatelessWidget {
  ...
  Widget _pairedImage(BuildContext context, List<String> urls) {
    return Row(
      children: urls.take(2).map((url) => Expanded(child: _image(context, url))).toList(growable: false),
    );
  }
